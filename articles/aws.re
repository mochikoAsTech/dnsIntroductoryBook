...
//image[signInMailAddress][AWSのアカウントのEメールアドレスを入力][scale=0.75]{
//}

続いてルートユーザーサインインの画面（@<img>{rootUserSignIn}）でパスワードを入力したら「サインイン」を押します。

//image[rootUserSignIn][パスワードを入力してサインイン][scale=0.75]{
//}

ログインするとマネジメントコンソールというAWSの管理画面（@<img>{managementConsole}）が表示されます。

//image[managementConsole][マネジメントコンソール（AWSの管理画面）][scale=0.75]{
//}
...
//image[searchDig][「dig」で検索][scale=0.75]{
//}
...
//image[digStartDnsFunByCman][自分のドメインを入力してネームサーバを調べる][scale=0.75]{
//}
...
//image[whichOneIsMyNameServer][自分のドメインを入力してネームサーバを調べる][scale=0.75]{
//}
...
//image[nsRecord][NSレコードのValueに書かれたネームサーバをメモしておく][scale=0.75]{
//}
...
//image[onamaeTopPageAgain][右上の「ドメインNavi ログイン」をクリック][scale=0.75]{
//}
...
//image[domainNaviLogin][お名前IDとパスワードを入れてドメインNaviにログイン][scale=0.75]{
//}
...
//image[continueContract][ドメインNavi ドメイン契約更新][scale=0.75]{
//}
...
//image[contractRemind][ドメインNavi 更新アラート][scale=0.75]{
//}
...
//image[domainNaviDomainList][ドメインNavi ドメイン一覧][scale=0.75]{
//}

ネームサーバー設定の画面（@<img>{nameServerSetting01}）を開いたら、少し下の「2.ネームサーバーの選択」までスクロールしてください。

//image[nameServerSetting01][ドメインNavi ネームサーバー設定][scale=0.75]{
//}
...
//image[nameServerSetting02][2.ネームサーバーの選択 変更前][scale=0.75]{
//}

//image[nameServerSetting03][2.ネームサーバーの選択 変更後][scale=0.75]{
//}

確認画面（@<img>{confirmNameServerSetting}）が出てくるので、ネームサーバ情報がRoute53のネームサーバになっていることを確認して「OK」を押します。

//image[confirmNameServerSetting][ネームサーバの設定確認][scale=0.75]{
//}
...
//image[nameServerSettingFinish][ネームサーバの設定変更完了][scale=0.75]{
//}
...
あなたは今、ドメインのネームサーバをデフォルト設定のお名前.comから、Route53のネームサーバに変更しました。1年後に「ドメインがもうすぐ有効期限だから更新しましょう！」と連絡してくるのはどちらですか？
