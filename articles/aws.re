...
アマゾンがやっているクラウドと言われても、そもそもクラウドってなんだろう？という方もいますよね。クラウドとは「ブラウザ上でスペックを選んでぽちぽちっとするだけで誰でもすぐにウェブサーバやDBサーバを立てたりネームサーバを使ったりできるサービス」@<fn>{cloud}のことです。AWSでは基本的に1秒単位の従量課金なのでたとえばサーバを立てて3分使ったら3分ぶんのお金しかかかりません。
...
TypeはPublic Hosted Zoneを選択（@<table>{hostedZones}）します。「Public Hosted Zone」にしておけば外から「このドメインに紐づいてるIPはなに？」と聞かれたときにRoute53のネームサーバが「IPは〇〇だよ」と返事をします。ここを「Private Hosted Zone for Amazon VPC」にしてしまうと、AWS内での名前解決しか出来なくなり外から聞かれても何も答えてくれなくなってしまいますので、必ず「Public Hosted Zone」にしておいてください。入力完了したら「Create」を押します。
...
これでRoute53というネームサーバの中に自分のドメインのゾーンが出来て、ゾーンの中にドメインのネームサーバを示すNSレコード（@<img>{nsRecord}）と管理情報を示すSOAレコードというリソースレコードも出来ました。
...
このようにお名前.comのネームサーバとRoute53のネームサーバ、どちらにもstartdns.funのゾーンがあるのですが、上位ネームサーバのa.nic.funで「startdns.funについてはdns1.onamae.comに委任する」という設定になっているため、お名前.comのネームサーバがstartdns.funの権威を持っており、startdns.funのNSレコードを問い合わせたときに誰もRoute53のネームサーバに聞きに来ない状態なのです。
...
正解はBです。今行った作業はドメインのネームサーバを、デフォルト設定のお名前.comからAWSのRoute53というネームサーバに変更しただけです。レジストラ（ドメインを購入するお店）をお名前.comからAWSのRoute53に変更する「レジストラ移管」を行ったわけではないので、1年後に更新確認の連絡をしてくるのもお名前.comです。
