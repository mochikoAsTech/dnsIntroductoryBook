...
@<chapref>{aws}ではAWSのRoute53というネームサーバを使って、自分のドメインのゾーンを作ってみましょう。
...
@<href>{https://aws.amazon.com/}の「コンソールへサインイン」を押すと、マネジメントコンソールへのログイン画面が表示されます。

先ほど登録したAWSアカウントのEメールアドレスとパスワードを入力して「サインイン」を押します。

ログインできたらマネジメントコンソールというAWSの管理画面が表示されます。

== お名前.comで買ったドメインのネームサーバをRoute53に変更

AWSのマネジメントコンソールで、左上のサービス＞ネットワーキング＞Route 53を選択してください。うまく見つからない時はRouteで検索してもいいです。
