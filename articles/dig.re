...
digコマンドやwhoisコマンドを使いたくてもインストールされていなければ使えません。Macユーザであれば元々インストールされているので、ターミナルを起動すればすぐに使うことができます。CentOSやAmazonLinuxなどのサーバ環境がある人は、そこでrootになって次のyumコマンドを叩けばインストールできます。
...
パソコンはWindowsだしコマンドを叩けるような環境はない・・・という方は、Route53を使うために折角AWSでアカウントを作ったので、EC2@<fn>{ec2}でインスタンス@<fn>{instance}を立ててみるのがお勧めです。サーバスペックごとにインスタンスタイプという区分があるのですが、t2.microというインスタンスタイプならアカウント作成から12か月間は毎月750時間まで無料で使えます。ここでは詳しく説明しませんが、AWSのマネジメントコンソールでEC2ダッシュボードを開いて「インスタンスの作成」を押したら、後は画面の表示に従ってぽちぽち選んでいくだけで5分もあればすぐにサーバが作れます。@<fn>{750h}

インスタンス立てるのはちょっと無理！という場合は、@<chapref>{aws}で使った「nslookup(dig)テスト【DNSサーバ接続確認】」@<fn>{cmanUrl3}というページや各レジストリのWhois情報検索サイトで疑似的にdigやwhoisを叩く形で代用しても構いません。
...
ただtechbookfest.orgに紐づくIPアドレスが知りたかっただけなのに、ものすごくいっぱい出てきました。digはまるでブキチ@<fn>{bukichi}のようなコマンドなのでちょっと聞いただけで「調べてきた結果を教えまし！まずキミの質問はこれでし！このドメインに紐づくIPはこれとこれとこれとこれなのでし！ちなみにこれを教えてくれたネームサーバの名前はこれでし、IPはこっちなのでし！ちなみにフルリゾルバは127.0.0.1で調査には148msecかかったのでし！」と調査の過程や付加情報まで含めて全部教えてくれます。
...
//footnote[bind][BINDはフルリゾルバとネームサーバ両方の機能を持つDNSサーバで、ISC（Internet Software Consortium）によって開発が行われています。多くのDNSサーバでBINDが採用されていますが「夏のBIND脆弱性祭り」などと揶揄されるほど脆弱性の注意喚起とそれに伴うアップデート推奨が多いため、最近はUnboundなど他DNSサーバへの乗り換えもよく聞かれます。]
...
IPアドレスは分かりましたが、IPだけではまだどこのクラウドサービスなのか分かりません。さらにwhoisコマンドでこのIPの持ち主を調べてみましょう。@<fn>{twoIp}

「whois 46.51.255.231」を叩いてみると「descr: Amazon Web Services, Elastic Compute Cloud, EC2」と出ますので、このIPはAWSのEC2のものであることが分かります。これでmojimo-mangaのサイトがAWSのEC2上で動いていることが分かりました。
...
MXレコードから察するに任天堂はOffice 365@<fn>{office365}を使っているようです。さらに「nintendo-co-jp.mail.protection.outlook.com」のAレコードを引くと、最終的にはメール受信サーバのIPまでたどり着くことができます。
...
ありがちですがお問い合わせフォームのメールアドレスの例にsample@test.comと書くのもやめましょう。銀行に行って申請用紙のサンプルに自分の住所が部屋番号までばっちり書かれていたらぎょっとしますよね？それと同じことです。実在する他人の住所や電話番号を勝手に使わないのと同じように、他人のドメインも勝手に使わないようにしましょう。
...
ゾーンの中のリソースレコードを確認するにはAWSのマネジメントコンソールにログインする必要がありますが、AWSアカウント名やパスワードまではさすがに分からないのでそこはなんとか過去資料から探してください。またRoute53にはホストゾーンのエクスポート機能がないので、リソースレコードを1つ1つコピーペーストするかCLI@<fn>{cli}を駆使して取得する必要があります。
...
このとき送り主が本物なのか、あるいは自分の持ち物でないドメインで送信元を詐称して勝手にメールを送っている「なりすましメール」なのか否かを確認する手段として、SPF@<fn>{spf}レコードというものがあります。SPFレコードは次のdigコマンドで確認できます。
...
あなたはWeb制作会社A社のフロントエンドエンジニアです。小規模なクライアントB社からの依頼でキャンペーンサイト（@<href>{http://campaign.example.com/}）を構築したのですが、後から「問い合わせフォームが欲しい」という追加希望が出てきました。
...
メールを受信するメールサーバによっては「メール送信元のIPがSPFレコードに登録されていること」だけでなく「メール送信元のIPからドメインの逆引きができること」という条件も満たさないと迷惑メールと判断することがあります。
...
なおAレコードやMXレコードやSPFレコードといったドメインのリソースレコードと、PTRレコードのようなIPのリソースレコードは設定依頼をする先が異なります。筆者はstartdns.funというドメインを持っており、ネームサーバはRoute53を使用しているので、startdns.funのAレコードやMXレコード、SPFレコードはRoute53で設定ができます。

ですが、たとえばさくらインターネットのVPSで借りたサーバのIPアドレスが203.0.113.222だったとして、このIPのPTRレコードをRoute53で設定することはできません。このIPに対してPTRレコードを設定できるのは、IPの持ち主であるさくらインターネットの管理画面からとなります。

=== CNAMEレコード

ときどきドメインからIPを引こうとしてdigでAレコードを調べたのに、こんな風にドメインとIPが返ってくることがあります。
...
ANSWER SECTIONを見てみましょう。aibo.sony.jpのCNAMEにcs1018.wpc.omicroncdn.netが設定されており、さらにcs1018.wpc.omicroncdn.netのAレコードに152.195.38.205が設定されていることが分かります。このとき「aibo.sony.jp」をaliases（別名）、「cs1018.wpc.omicroncdn.net」をcanonical name（正式名）と呼びます。
