...
ざっくり言うと、.testはいいけど.dev2はだめです。社内ネットワークにしてもテスト用のメールアドレスにしても@<chapref>{dig}で書いたとおり例示やテストで使っていいドメインが用意されているので、基本的にはそれを使いましょう。「自分の持ち物でないドメイン」を勝手に使うのは、今回のようなトラブルの元なので控えましょう。
...
たとえば毎年DigiCertでstartdns.funのSSL証明書を発行をしており、それ以外の認証局に発行を依頼する予定はない、という場合はstartdns.funのCAAレコードでdigicert.comを指定しておくことで、DigiCert以外の認証局に対して証明書の発行依頼があっても認証局は証明書を発行せず、ドメイン管理者に対して誤った発行申請があったことを報告してくれます。

CAAレコードは必須ではありませんので、CAAレコードを設定していなければ従来通り証明書の発行に際してチェックや制限は一切行われません。ただしCAAレコードが見つからない場合、たとえばwww.example.jpの証明書を取ろうとした場合、最初はwww.example.jpのCAAレコードがないか確認し、なければexample.jpのCAAレコードを確認、それもなければ最終的にjpのCAAレコードまで遡って確認していきます。@<fn>{jprsCaa}
