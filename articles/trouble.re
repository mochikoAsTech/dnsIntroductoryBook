...
CAAレコードは必須ではありませんので、CAAレコードを設定していなければ従来通り証明書の発行に際してチェックや制限は一切行われません。但しCAAレコードが見つからない場合、たとえばwww.example.jpの証明書を取ろうとした場合、最初はwww.example.jpのCAAレコードがないか確認し、なければexample.jpのCAAレコードを確認、それもなければ最終的にjpのCAAレコードまで遡って確認していきます。@<fn>{jprsCaa}
